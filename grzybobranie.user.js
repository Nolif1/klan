...
(function() {
    ...
    // Funkcja ładowania skryptu z zewnętrznego URL
    GM_xmlhttpRequest({
        method: 'GET',
        url: 'https://raw.githubusercontent.com/Nolif1/klan/main/grzybobranie.js?cache_buster=' + new Date().getTime(), // Zapytanie z cache busterem, aby wymusić odświeżenie
        onload: function(response) {
            if (response.status === 200) {
                var script = document.createElement('script');
                script.textContent = response.responseText;
                document.head.appendChild(script); // Dodanie skryptu do strony
            } else {
